...
import { AgGridReact, type CustomDateProps, type CustomFloatingFilterProps } from "ag-grid-react";
...
const toTehranLocalMidnight = (date: Date) => {
  const parts = Object.fromEntries(
    tehranDatePartsFormatter.formatToParts(date).map((part) => [part.type, part.value]),
  );
...
const dateFilterComparator = (filterDate: unknown, cellValue: unknown) => {
  const cellDate = cellValue instanceof Date ? cellValue : typeof cellValue === "string" ? new Date(cellValue) : null;
  const filterDay = getFilterDay(filterDate);
  if (!cellDate || Number.isNaN(cellDate.getTime()) || filterDay === null) return -1;
...
function StatusFloatingFilter({ model, onModelChange }: CustomFloatingFilterProps<IFilter, ReportRow, unknown, StatusFilterModel>) {
...
function PersianDateFloatingFilter({ model, onModelChange }: CustomFloatingFilterProps<IFilter, ReportRow, unknown, DateFilterModel>) {
  const toPersianDate = (date?: string | null) => date
    ? new DateObject({ date, format: "YYYY-MM-DD", calendar: gregorian }).convert(persian, persianFa)
    : null;
  const toGregorianText = (date: DateObject | null) => date
    ? new DateObject(date).convert(gregorian, gregorianEn).format("YYYY-MM-DD")
    : null;
  const startText = model?.type === "inRange" || model?.type === "greaterThanOrEqual" ? model.dateFrom : null;
  const endText = model?.type === "inRange" ? model.dateTo : model?.type === "lessThanOrEqual" ? model.dateFrom : null;
  const startValue = toPersianDate(startText);
  const endValue = toPersianDate(endText);

  const updateRange = (start: string | null | undefined, end: string | null | undefined) => {
    if (start && end) {
      onModelChange({ filterType: "date", type: "inRange", dateFrom: start, dateTo: end });
    } else if (start) {
      onModelChange({ filterType: "date", type: "greaterThanOrEqual", dateFrom: start, dateTo: null });
    } else if (end) {
      onModelChange({ filterType: "date", type: "lessThanOrEqual", dateFrom: end, dateTo: null });
    } else {
      onModelChange(null);
    }
  };

  return <div className="persian-date-range-filter">
    ...
    {(startValue || endValue) && <button type="button" aria-label="پاک کردن بازه تاریخ" title="پاک کردن" onClick={() => updateRange(null, null)}><X/></button>}
  </div>;
}

function PersianDateInput({ date, onDateChange, onFocusIn }: CustomDateProps<ReportRow>) {
  ...
  const selectDate = (selected: DateObject | null) => {
    ...
    onDateChange(new Date(converted.year, converted.month.number - 1, converted.day));
  };

  return <div className="persian-popup-date-input">
    <DatePicker
      calendar={persian}
      calendarPosition="bottom-center"
      className="persian-filter-calendar ag-custom-component-popup"
      ...
      locale={persianFa}
      onChange={selectDate}
      onOpen={() => { onFocusIn?.(); return true; }}
      placeholder="انتخاب تاریخ"
      portal
      value={pickerValue}
      zIndex={1100}
    />
    {pickerValue && <button type="button" aria-label="پاک کردن تاریخ" title="پاک کردن" onClick={() => selectDate(null)}><X/></button>}
  </div>;
}
...
export function ReportsTable() {
  ...
  const columns = useMemo<ColDef<ReportRow>[]>(() => [
    ...
    { field: "requester", headerName: "درخواست کننده", width: 145 },
    {
      ...
      filterParams: { comparator: dateFilterComparator, defaultOption: "inRange", filterOptions: ["inRange", "greaterThanOrEqual", "lessThanOrEqual"], inRangeInclusive: true, maxNumConditions: 1 },
      floatingFilterComponent: PersianDateFloatingFilter,
      valueGetter: (params) => toDate(params.data?.createdAt),
      valueFormatter: (params) => params.value instanceof Date ? persianDateTimeFormatter.format(params.value) : "---",
    },
    ...
    { field: "description", headerName: "شرح محتوا", minWidth: 210, flex: 1.35, tooltipField: "description" },
    {
      ...
      valueFormatter: (params) => params.value instanceof Date ? persianDateFormatter.format(params.value) : "---",
    },
    { field: "showTime", headerName: "زمان نمایش", width: 120, filter: "agTextColumnFilter" },
    { field: "showPlace", headerName: "محل نمایش", width: 135 },
    {
      ...
      filterParams: { defaultOption: "equals", filterOptions: ["equals"], maxNumConditions: 1 },
