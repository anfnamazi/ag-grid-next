...
import { ArrowIcon, BellIcon, ChevronIcon, Icon, type IconName, MenuIcon, MoonIcon, MoreIcon, PlusIcon, SearchIcon, SunIcon, TrendIcon } from "./icons";
...
function Brand() {
  return <div className="brand" aria-label="همراه‌پنل"><span className="brand-mark" aria-hidden="true"><span/><span/><span/></span><span className="brand-copy"><strong>همراه‌پنل</strong><small>مدیریت یکپارچه</small></span></div>;
}
...
function Sidebar({ activePath, onClose }: { activePath: string; onClose: () => void }) {
  ...
  return <aside className="sidebar" aria-label="منوی اصلی">
    <div className="sidebar-brand"><Brand/><Button aria-label="بستن منو" className="sidebar-close" isIconOnly variant="ghost" onPress={onClose}>×</Button></div>
    <div className="search-box"><SearchIcon className="search-icon"/><Input ref={searchRef} aria-label="جستجو در منو" fullWidth placeholder="جستجو در منو..." value={query} onChange={(event) => setQuery(event.target.value)} variant="secondary"/><kbd>⌘ K</kbd></div>
    <nav className="nav-groups">
      {filteredGroups.length ? filteredGroups.map((group) => <div className="nav-group" key={group.title}>
        <p>{group.title}</p>
        {group.items.map((item) => {
          const content = <><Icon name={item.icon}/><span>{item.label}</span>{item.count && <span className="nav-count">{item.count}</span>}{activePath === item.href && <ChevronIcon className="nav-chevron"/>}</>;
          return item.href
    ...
    <div className="sidebar-upgrade"><span className="spark">✦</span><strong>امکانات بیشتر می‌خواهید؟</strong><p>با ارتقا به نسخه حرفه‌ای، محدودیت‌ها را کنار بگذارید.</p><Button fullWidth size="sm" variant="primary">مشاهده پلن‌ها</Button></div>
    <div className="sidebar-user"><UserAvatar/><div><strong>مهدی نادری</strong><span>مدیر محصول</span></div><Button aria-label="گزینه‌های حساب" isIconOnly size="sm" variant="ghost"><MoreIcon/></Button></div>
  </aside>;
}

function Header({ currentPage, onTheme, onMenu }: { currentPage: string; onTheme: () => void; onMenu: () => void }) {
  return <header className="topbar">
    <div className="mobile-brand"><Button aria-label="باز کردن منو" isIconOnly variant="ghost" onPress={onMenu}><MenuIcon/></Button><Brand/></div>
    <div className="topbar-title"><span>داشبورد</span><ChevronIcon/><strong>{currentPage}</strong></div>
    <div className="header-actions">
      <Button aria-label="تغییر پوسته روشن یا تاریک" className="header-icon-button" isIconOnly variant="ghost" onPress={onTheme}><SunIcon className="theme-sun"/><MoonIcon className="theme-moon"/></Button>
      <Button aria-label="اعلان‌ها" className="header-icon-button notification-button" isIconOnly variant="ghost"><BellIcon/><span/></Button>
      <div className="header-divider"/><button type="button" className="profile-button"><UserAvatar/><span><strong>مهدی نادری</strong><small>مدیر محصول</small></span><ChevronIcon/></button>
    </div>
  </header>;
}

function StatCard({ item }: { item: (typeof stats)[number] }) {
  return <article className={`stat-card stat-${item.color}`}><div className="stat-top"><span>{item.label}</span><button aria-label={`گزینه‌های ${item.label}`} type="button"><MoreIcon/></button></div><strong className="stat-value">{item.value}</strong><div className="stat-meta"><span><TrendIcon/>{item.change}</span><small>{item.note}</small></div><div className="stat-decoration"><i/><i/><i/></div></article>;
}
...
function RevenueChart() {
  return <section className="panel chart-panel">
    ...
    <div className="chart-summary"><strong>۱٫۲ میلیارد تومان</strong><span><TrendIcon/> ۱۸٫۲٪ رشد</span></div>
...
function ActivityPanel() {
  return <section className="panel activity-panel"><div className="panel-heading"><div><h2>فعالیت‌های اخیر</h2><p>آخرین تغییرات تیم شما</p></div><button type="button" className="text-link">مشاهده همه <ArrowIcon/></button></div><div className="activity-list">
